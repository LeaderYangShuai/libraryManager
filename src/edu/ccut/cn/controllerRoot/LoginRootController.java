...
/**
 * Servlet implementation class LoginRootController
 */
@WebServlet("/LoginRootController")
public class LoginRootController extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		if (rootOperation.rootIsOk(rootName, rootPassword)) {
			request.getRequestDispatcher("/skipRoot.jsp?path=loginManager").forward(request, response);
		}else {
			request.setAttribute("content", "不要非法进入");
			request.getRequestDispatcher("/skipRoot.jsp?path=loginRoot").forward(request, response);
		}
